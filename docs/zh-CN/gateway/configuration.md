...
OpenClaw 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.accounts.default.botToken`（或 `channels.telegram.accounts.default.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退，额外账号可在 `channels.telegram.accounts.<id>` 下定义。
